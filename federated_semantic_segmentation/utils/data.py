...
# to_pytorch ora gestisce solo la normalizzazione
...
class SelmaDrones(Dataset):
    ...
    def __getitem__(self, idx: int):
        fbase, (h, t) = self.items[idx]   # h è già float (metri), t è string
        ...
        dth = cv.resize(dth, TARGET_SIZE, interpolation=cv.INTER_AREA)
        ...
        # Converte in torch (RGB normalized + depth concatenata)
        x4, mlb_t = to_pytorch(rgb, dth, mlb)   # x4: [4,H,W], mlb_t: [H,W]

        # Altitudine come tensore float32 [1]
        alt_t = torch.tensor(float(h), dtype=torch.float32)

        # Meta-info opzionale (per logging/analisi)
        meta = {"height_m": float(h), "town": t}

        return x4, mlb_t, alt_t, meta

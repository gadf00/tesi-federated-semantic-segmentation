...
DEFAULT_MODEL_PATH = "checkpoints/iid_100ls_40tr_30cl_last_1.pt"
DEFAULT_OUTPUT_DIR = "iid_all_100ls_40tr_30cl_1"
...
def run_offline_analysis(model_path: str, output_dir: str):
    ...
    1. Valuta il modello sul test set.
    2. Salva immagini RGB/GT/Pred non stretchate.
    ...
    # 3. Carica i Dati di Test (stesso loader del server)
    try:
        val_loader = load_server_data()
        cnames = getattr(val_loader.dataset, "label_names", None)
    ...
    with torch.no_grad():
        # IMPORTANTE: il loader restituisce (x4, mlb, alt, _meta)
        for i, (x4, mlb, alt, _meta) in enumerate(val_loader):

            x4  = x4.to(device, dtype=torch.float32)   # [B, C, H, W] (RGB + depth [+ alt map interna])
            mlb = mlb.to(device, dtype=torch.long)     # [B, H, W]
            alt = alt.to(device, dtype=torch.float32)  # [B]

            # Forward del modello: stessa firma di train/test in task.py
            logits = model(x4, alt)
            ...
            if i < NUM_IMAGES_TO_LOG:
                try:
                    # Per la visualizzazione usiamo solo i primi 3 canali come RGB
                    rgb_tensor = x4[0, :3, :, :]
                    gt_mask = mlb[0]
                    pred_mask = preds[0]
                    ...
                    # Salvataggio su disco in BGR per OpenCV
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script di Analisi e Visualizzazione Offline"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            "Cartella dove salvare il report.txt e le immagini "
            "(rgb_images/, gt_labels/, pred_predictions/)."
        ),
    )
